  ...
  const saveNewImage = async (result: ImagePicker.ImagePickerResult) => {
    ...
    if (!result.canceled) {
        ...
        if (Platform.OS !== 'web') {
          ...
          localStorage.setItem('plant_image', result?.assets[0]?.uri);
          // For web, we don't need to delete the old image as it is not stored in the file system
        }
    }
  }
  ...
  return (
    <LinearGradient colors={['#4CAF50', '#000000']} style={{ flex: 1, justifyContent: 'space-around', alignItems: 'center', padding: 20, flexDirection: 'column', backgroundColor: '#000000' }}>
      ...
      <>
        ...
        <TouchableOpacity onPress={analyzeImage} style={{ backgroundColor: 'white', flexDirection: 'row', alignItems: 'center', padding: 12, borderRadius: 10, marginTop: 10, gap: 10 }} disabled={!imageUri}>
